...
# create the jobs
def createJob(jobs, py, te, cDna, ncRna, prd, read_leng, strandn, cpu, remove, indicix,queue,wallt,mdl,mm):
  job_list = []
  for i in range(0, int(jobs)):
    with open("job"+str(i), 'w') as outj:
      ...
      outj.write("time python3 "+py+" --TE "+te+" --cdna "+cDna+" --ncrna "+ncRna+" --sample sample"+str(i)+".txt --paired "+prd+" --length "+str(read_leng)+" --out "+str(i)+ " --strand "+strandn+ " --num_threads "+str(cpu)+" --remove "+remove+ " --index "+indicix+ " --multimap "+str(mm)+"\n\n")
